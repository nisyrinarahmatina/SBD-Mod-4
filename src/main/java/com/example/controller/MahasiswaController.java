...
@GetMapping("/add")
    ...
    public String add(Mahasiswa mahasiswa) {
        ...
        jdbcTemplate.update(sql, mahasiswa.getNim(),
                mahasiswa.getNama(), mahasiswa.getAngkatan(), mahasiswa.getGender(), mahasiswa.getJurusan(),
    ...
    public String edit(Mahasiswa mahasiswa) {
        String sql = "UPDATE mahasiswa SET nama = ?, angkatan = ?, gender = ?  , jurusan = ?, fakultas = ? WHERE nim = ?";
        jdbcTemplate.update(sql, mahasiswa.getNama(), mahasiswa.getAngkatan(), mahasiswa.getGender(),
         mahasiswa.getJurusan(), mahasiswa.getFakultas() , mahasiswa.getNim());
        return "redirect:/";
    }
